...
class TinyAppException : public std::exception
{
public:
	TinyAppException(int line, const char* file) noexcept :
		m_line(line),
		m_file(file)
	{}
	TinyAppException(const TinyAppException&) = delete;
	TinyAppException& operator=(const TinyAppException&) = delete;
	virtual ~TinyAppException() noexcept override {}
